...
16. **Raw exceptions in snackbars** — `bills_screen.dart:860,894,934`, `pay_runs_screen.dart:260`, `gst_returns_screen.dart:46`, etc.: `"Couldn't approve: $e"`. Map to friendly messages. — ✅ DONE (2026-07-12)
    **Done:** split the generic `catch (e)` snackbars in `bills_screen.dart` (approve/delete/markPaid) and `pay_runs_screen.dart` (schedule) into `on ApiException → e.message` / `catch (_) → friendly fallback`. `gst_returns_screen.dart` already used a `friendlyGstError` helper.
17. **Inconsistent dashboard error handling** — SpotlightCards silently blank on error (`spotlight_cards.dart:28`); recent lists have no retry; ActivityList has full retry. Standardize on `AsyncSlot`. — ✅ DONE (2026-07-12)
    **Done:** `spotlight_cards.dart` and `recent_lists.dart` (RecentInvoices/RecentBills) now use `AsyncSlot` with `onRetry` invalidating their providers, so errors show a Retry affordance instead of blanking. Removed the dead `_ErrorRow`.
18. **Header stat chips sum only the loaded page (max 50)** — `invoices_screen.dart:124-158`; understates OUTSTANDING/COLLECTED on big books. Serve true totals from the summary endpoint. — ⬜ DEFERRED (needs backend)
    **Why:** OUTSTANDING is already fixable (`InvoiceSummary.totalOutstanding` exists), but COLLECTED and DRAFT have no all-time field (`receivedThisMonth` is month-scoped). An honest fix needs new backend summary fields (all-time collected, draft amount) and should cover both invoices AND bills header chips consistently — a backend+frontend change, not a partial frontend patch that leaves some chips silently wrong.
19. **Banking is web-dependent** — `banking_screen.dart:66,171`: empty states punt to "the web app" for adding accounts/importing statements. At minimum let mobile share-in a statement PDF (the share-intake pipeline already exists). — ⬜ DEFERRED (feature)
    **Why:** statement-PDF ingest is a real feature (parse route + intake destination + reconciliation wiring); belongs in the "Structural" roadmap phase, not this polish pass.
20. **Share-in always interrupts with a 4-way jargon sheet** — `share_destination_sheet.dart` forces "Vendor bill / Customer PO / Receive against PO (GRN) / Quick payment" on every share. Default to "Vendor bill" with a one-tap switch. — ✅ DONE (2026-07-12)
    **Done:** the sheet now leads with a prominent primary "Vendor bill" tile ("Most shares are a supplier's invoice…") and hides the other three behind a "Not a vendor bill?" expander. All four destinations still reachable; enum + `showShareDestinationSheet()` signature unchanged.
21. **No offline story** — all writes fail hard with a generic error; no queue/retry for shop-floor connectivity. — ⬜ DEFERRED (architectural)
    **Why:** an offline write-queue with retry/conflict handling is a cross-cutting architecture project (the audit itself sequences it under "Structural — as roadmap"), not a polish fix.
22. **Money hub duplication** — hero duplicates dashboard's (minus AR/AP stats); 4 top chips duplicate 4 of the 7 grid tiles (`money_hub_screen.dart:42-65` vs `:231-302`). Cut the chips, keep the grid. — ✅ DONE (2026-07-12)
    **Done:** removed the 4-chip row (a confirmed subset of the 7 grid tiles) and its now-unused import from `money_hub_screen.dart`, tightened the spacing. Kept the hero and the grid; the shared `HubQuickChip` widget stays (still used by sales/purchases hubs).
23. **Reports numbers aren't tappable** — `reports_screen.dart:424,731`: P&L rows, top customers, top categories have no drill-down, while Analytics cards do. Add taps → filtered lists. — ⬜ DEFERRED (feature)
    **Why:** requires filtered list-screen targets that accept the right query params (customer/category/GL) and per-row routing; a feature slice worth its own pass + verify, not a quick polish edit.
24. **No period selector on Analytics** — sections hardcode FY / 12mo / 90d / this-month; only the P&L card has its own toggle (`section_books.dart:146-151`). — ⬜ DEFERRED (feature)
    **Why:** every analytics provider is a `.family` keyed on its own hardcoded period; a global selector means threading a shared period through all section providers (cash, AR/AP, performance, GST) — a real refactor, not polish.
