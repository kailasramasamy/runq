// Admin/manager sheet to override an employee's leave balance for a
...
/// One-call entry point. Routes the user through employee picker → type
/// picker → opening/accrued sheet, all stitched together via this state
/// container. Returns once the user saves or cancels.
Future<void> showAdjustLeaveBalanceSheet(BuildContext context) {
...
class _AdjustSheetState extends ConsumerState<_AdjustSheet> {
  ...
  Future<void> _maybePrefill() async {
    ...
    try {
      ...
      final match = balances.where((b) => b.leaveTypeId == _type!.id).cast<HrLeaveBalance?>().firstWhere(
            (_) => true,
            orElse: () => null,
          );
      if (!mounted) return;
