...
import { Bot, Coins, GitBranch, RefreshCcw, Shield, Sparkles } from 'lucide-react';
...
interface Row { date: string; desc: string; amt: string; match: string; conf: number }
          ...
          <tbody className="tabular">
            {rows.map((r, i) => (
              <tr key={i} className="border-t border-white/5">
                ...
                <td className={`px-4 py-3 text-right font-medium ${r.amt.startsWith('+') ? 'text-emerald-400' : 'text-zinc-200'}`}>
                  {r.amt}
                </td>
                <td className="px-4 py-3">
                  {r.conf > 0 ? (
                    <span className="inline-flex items-center gap-1.5">
                      <span
                        className={
                          r.conf > 95 ? 'inline-flex h-1.5 w-1.5 rounded-full bg-emerald-400'
                          : r.conf > 90 ? 'inline-flex h-1.5 w-1.5 rounded-full bg-emerald-400/70'
                          : 'inline-flex h-1.5 w-1.5 rounded-full bg-amber-400'
                        }
                      />
...
const why = [
  { Icon: Sparkles,   title: 'Trained on Indian narration',  body: 'NEFT/IMPS/UPI codes, PSU bank quirks, abbreviations like "INF/" and "TRTR/" — handled natively.' },
  { Icon: RefreshCcw, title: 'Recurring detection',          body: 'Rent, utilities, salaries — runQ learns the pattern after two cycles and auto-categorises every following one.' },
  { Icon: GitBranch,  title: 'Split & part-payments',        body: 'A single bank credit can settle three invoices. A single payout can cover four bills. Splits are first-class.' },
  { Icon: Coins,      title: 'Multi-bank, multi-currency',   body: 'Reconcile rupee accounts and EEFC/USD accounts side-by-side. Forex differences booked correctly.' },
  { Icon: Shield,     title: 'Read-only access',             body: 'Bank credentials never touch our servers. Connections are revocable from your bank portal at any time.' },
  { Icon: Bot,        title: 'Smart suggestions',            body: 'Unmatched lines get an AI explanation: "Looks like a refund to vendor Patel Tools — book against bill #B-42?"' },
];

export default function BankReconPage() {
  useEffect(() => { document.title = 'Bank Reconciliation — runQ'; }, []);
  return (
    <main>
      <PageHero
      ...
      <Section>
        <SectionHead
          ...
          title="Built for Indian bank statement noise."
          subtitle="Most matchers fail on Indian narration codes. Ours is trained on millions of them."
        />
        <div className="mt-12">
          <IconGrid items={why} />
        </div>
      ...
      <DarkSection>
        <SectionHead eyebrow="THE NUMBERS" title="What our beta books look like." accent="violet" dark />
        <div className="reveal mt-10 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-4">
          {([
            ['94.3%',   'Avg auto-match rate'],
            ['6.2 hrs', 'Saved per week'],
            ['60+',     'Banks supported'],
            ['<6 hrs',  'Statement sync interval'],
          ] as const).map(([v, k]) => (
