...
import { useDn, useUpdateDn, useWarehouses } from '@/hooks/queries/use-inventory';
...
export function EditDeliveryNotePage() {
  ...
  useEffect(() => {
    if (!dn || hydrated) return;
    setWarehouseId(dn.warehouseId);
    setCustomerId(dn.customerId ?? '');
    setDispatchDate(dn.dispatchDate);
    setVehicleNo(dn.vehicleNo ?? '');
    setEWayBillNo(dn.eWayBillNo ?? '');
    setNotes(dn.notes ?? '');
    setLines(
      dn.lines.length
        ? dn.lines.map((l) => ({ itemId: l.itemId, batchNo: l.batchNo ?? '', qty: Number(l.qty) }))
        : [emptyLine()],
    );
    setHydrated(true);
  }, [dn, hydrated]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      navigate({ to: '/inventory/delivery/$id', params: { id } });
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader title={`Edit ${dn.dnNo}`} description="Update draft delivery before dispatch." />
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            type="button"
            variant="secondary"
            onClick={() => navigate({ to: '/inventory/delivery/$id', params: { id } })}
          >
            Cancel
          </Button>
        </div>
