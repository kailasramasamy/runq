...
import { useImportCustomersCSV } from '@/hooks/queries/use-customers';
...
const CSV_FORMAT = `Name,Type,Phone,Email,GSTIN,PAN,Address,City,State,Pincode,Payment Terms,Contact Person
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <PageHeader
        ...
        description="Paste CSV data to bulk import or update customers."
      />
              ...
              <tbody>
                {preview.slice(0, 20).map((row, i) => (
                  <tr key={i} className="border-b border-zinc-100 last:border-0 dark:border-zinc-800">
                    ...
                    <td className="px-4 py-2 text-zinc-500">{row.email || '—'}</td>
      ...
      {step === 3 && result && (
        <Card>
          ...
          <CardFooter className="flex justify-end">
            <Button onClick={() => navigate({ to: '/finance/ar/customers' })}>View Customers</Button>
          </CardFooter>
        </Card>
      )}
    </div>
