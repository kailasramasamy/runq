#!/usr/bin/env bash
# Build the Dhenu iOS release and upload it to App Store Connect in one step.
#
...
# window to keep open.
#
# Dhenu ships under com.quartex.dhenu — a different app record, but the same
# Apple account as runq, so the same API key signs both. The key identifiers
# are baked in below; the only per-machine requirement is the private key:
...
#
# Unlike runq this archives a single target, so signing is one provisioning
# profile. If it is missing the build fails here rather than silently
...
# The two apps' scripts are near-identical, and running one from the other's
# directory would upload the wrong binary under the right version number.
[[ -f pubspec.yaml && -d ios ]] || die "run this from apps/collect/"
grep -q '^name: *dhenu' pubspec.yaml || die "this is not apps/collect/ — that's runq's script"
...
echo "release-ios: dhenu ${VERSION_LINE}${BUILD_NUMBER:+ (build number overridden to $BUILD_NUMBER)}"
...
if [[ "$UPLOAD" == 1 ]]; then
  ...
  step "Done"
  echo "Uploaded dhenu ${VERSION_LINE}. App Store Connect takes 5-15 minutes to"
  echo "process it, then it shows up in TestFlight for internal testers."
  echo
