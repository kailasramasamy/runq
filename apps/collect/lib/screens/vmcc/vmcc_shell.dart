...
/// VMCC operator shell — wraps the 4-tab role experience.
/// Home / ➕ Collect / Dispatch / Payments. Farmers & Reports are reached as
/// quick-links from Home (today-summary lives on Home now), and Profile from
/// the Home avatar — it is occasional business, not the daily loop.
class VmccShell extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final toDispatch = owed.where((s) => s.closed).length;

    final items = [
      DhenuNavItem(icon: DhenuIcons.home, label: l.navHome),
      DhenuNavItem(icon: DhenuIcons.collect, label: l.navCollect, count: toClose),
      DhenuNavItem(icon: DhenuIcons.dispatch, label: l.navDispatch, count: toDispatch),
