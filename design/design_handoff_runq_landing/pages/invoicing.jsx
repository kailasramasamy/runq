...
const { FeatureRow, IconGrid, Section, GreySection, DarkSection, SectionHead, StatsStrip } = window.Blocks;

// Mini invoice mockup — reused / adapted from main landing
function InvoiceMockup() {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-1.5 shadow-xl shadow-brand-500/10 mockup-shadow-light">
      <div className="rounded-xl bg-white">
        <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-3">
          ...
          <div className="flex items-center gap-2 text-xs text-zinc-500"><I.Sparkle size={12} className="text-brand-500" /> AI assist on</div>
        </div>
        <div className="grid grid-cols-12 gap-5 p-6">
                  ...
                  <tbody className="tabular">
                    {[
                      ...
                      <tr key={i} className="border-t border-zinc-200">
                        {r.map((c, j) => <td key={j} className={`px-3 py-2 ${j === 0 ? 'text-zinc-900' : 'text-right text-zinc-700'}`}>{c}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="col-span-5 space-y-4">
            ...
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs">
              <div className="flex items-center gap-1.5 font-semibold text-emerald-700"><I.CheckCircle size={13} /> e-Invoice generated</div>
            ...
            <button className="w-full rounded-md bg-zinc-900 px-3 py-2 text-xs font-medium text-white">Send via WhatsApp + email →</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <main>
        ...
        <GreySection>
          <SectionHead eyebrow="WHAT YOU GET" title="Everything that should be one click, is." subtitle="No hunting through menus to e-sign, no separate portal for e-Way." />
          <div className="mt-12">
            <IconGrid items={[
              { Ic: I.Receipt, title: 'GST-compliant invoices', body: 'Automatic CGST/SGST/IGST split. State-aware tax rules. HSN/SAC suggestions on every item.' },
              { Ic: I.ShieldCheck, title: 'e-Invoice (IRN) included', body: 'IRN generated on the IRP in the background. QR + acknowledgement embedded on the PDF.' },
              { Ic: I.GitBranch, title: 'e-Way bill, automatic', body: 'Above ₹50,000 we generate the e-Way bill alongside the invoice. Vehicle number can be added later.' },
              { Ic: I.Sparkle, title: 'AI item suggestions', body: 'Type a few characters of an item — runQ remembers HSN, last rate, and the right tax slab.' },
              { Ic: I.Mobile, title: 'Send via WhatsApp', body: 'One tap: WhatsApp opens with the customer, the PDF, a payment link, and a polite reminder template.' },
              { Ic: I.Coins, title: 'Built-in payment links', body: 'UPI, card, netbanking, and QR — collected via a runQ-managed payment page. Auto-marked paid on receipt.' },
              { Ic: I.Hash, title: 'Multi-series, multi-GSTIN', body: 'Separate invoice series per branch and GSTIN. Auto-locking once filed in GSTR-1.' },
              { Ic: I.Refresh, title: 'Recurring invoices', body: 'Set up monthly retainers and AMCs once — runQ raises, sends, and reconciles them every cycle.' },
              { Ic: I.FileText, title: 'Customisable templates', body: 'Upload your logo, pick a theme. Or use one of the eight templates designed for Indian formats.' },
            ]} />
          </div>
        </GreySection>
                  ...
                  <div className="overflow-hidden rounded-[34px] bg-white">
                    ...
                    <div className="space-y-2 px-5 py-4">
                      ...
                      <div className="rounded-lg bg-zinc-50 px-3 py-2.5 text-xs tabular">
                        <div className="flex justify-between text-zinc-500"><span>Total with GST</span><span className="font-semibold text-zinc-900">₹20,956.80</span></div>
                      </div>
                      <button className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-3 text-xs font-semibold text-white">Send via WhatsApp</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <DarkSection>
          <SectionHead eyebrow="UNDER THE HOOD" title="Compliance built-in, not bolted on." accent="violet" />
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              { t: 'IRP integration', b: 'Direct connection to the GSTN Invoice Registration Portal. No third-party intermediaries.' },
              { t: 'Reverse charge & composition', b: 'Native handling for RCM transactions, composition scheme, and unregistered customers.' },
              { t: 'SEZ & exports', b: 'LUT, with-payment / without-payment, foreign currency invoicing — all GST-correct.' },
            ].map((x, i) => (
              <div key={i} className="reveal rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
                <div className="text-base font-semibold text-white">{x.t}</div>
                <div className="mt-2 text-sm text-zinc-400">{x.b}</div>
              </div>
            ))}
          </div>
          <div className="reveal mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-4">
            {[['28s', 'Average invoice creation'], ['99.7%', 'IRN success on first try'], ['8', 'Invoice templates'], ['0', 'Per-invoice fees']].map(([v, k]) => (
